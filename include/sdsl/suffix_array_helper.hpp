...
struct traverse_csa_psi_trait {
	...
	static value_type access(const t_csa& csa, size_type i) { return csa.psi[i]; }
};
...
public:
	...
	typedef typename t_csa::size_type		size_type;
	typedef typename t_csa::difference_type difference_type;
	typedef random_access_const_iterator<traverse_csa_saisa>
							 const_iterator; // STL Container requirement
	typedef csa_member_tag   category;
	typedef int_alphabet_tag alphabet_category;
...
private:
	const t_csa&
	m_csa; //<- pointer to the (compressed) suffix array that is based on the \f$\Psi\f$ function.
public:
	//! Constructor
...
private:
	const t_csa&
	m_csa; //<- pointer to the (compressed) suffix array that is based on a wavelet tree
