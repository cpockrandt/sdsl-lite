...
void test_InsertAndDelete(uint64_t size, uint8_t width)
{
    ...
    value_type val2 = rng() & sdsl::bits::lo_set[width];
    {
        // insert(it, value) and emplace(it, value)
        t_iv iv(size, val1, width);
        ...
        iv.emplace(iv.end(), val2); // insert at end
        ASSERT_EQ(iv.size(), size + 4);
        for (size_type j=0; j < iv.size(); ++j) {
            if (j == 0 || j == 2 || j >= iv.size() - 2) {
                ASSERT_EQ(val2, iv[j]);
            } else {
                ASSERT_EQ(val1, iv[j]);
            }
    ...
    {
        // push_back(value) and emplace_back(value)
        t_iv iv(size, val1, width);
        iv.push_back(val2);
        iv.emplace_back(val2);
        ASSERT_EQ(iv.size(), size + 2);
        for (size_type j=0; j < iv.size(); ++j) {
            if (j >= iv.size() - 2) {
                ASSERT_EQ(val2, iv[j]);
